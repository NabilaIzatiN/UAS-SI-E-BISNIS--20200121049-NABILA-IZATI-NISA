...
@section('container')
    ...
    <h1 class="mb-3">List Jadwal Mata Kuliah</h1>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="btn-group" role="group">
                            <a href="/jadwal/{{ $item->id }}/edit" class="btn btn-warning btn-sm text-dark">Edit</a>
                            {{-- create small space between button --}}
                            &nbsp; | &nbsp;
                            <form action="/jadwal/{{ $item->id }}" method="post">
                                @method('DELETE')
                                @csrf
                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda Ingin Menghapus Data Ini ? {{ $item->nama_mhs }}')">Delete</button>
                            </form>
                        </div>
                    </td>
                </tr>
            ...
            <form action="{{ url('jadwal') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="matakuliah_id" aria-label="Default select example">
                        <option disabled value selected>Pilih Mata Kuliah</option>
